...
import { Box, Typography, TextField, IconButton, Tooltip, Dialog, DialogTitle, DialogContent, DialogActions, Button } from '@mui/material'
import { DatePicker } from '@mui/x-date-pickers/DatePicker'
import { format } from 'date-fns'
import AddIcon from '@mui/icons-material/Add'
import { useSelector, useDispatch } from 'react-redux'
import TaskList from './TaskList'
import { addTask, deleteTask, toggleComplete, editTask, addSubtask, toggleSubtask, deleteSubtask, editSubtask, assignDate } from '../redux/slices/tasksSlice'
...
export default function Backlog() {
  ...
  function handleAdd() {
    ...
    dispatch(addTask({ title: title.trim(), description: desc.trim(), backlog: true }))
    ...
    <Box>
      ...
      {backlog.length === 0 ? (
        ...
        <TaskList
          ...
          onEdit={(id) => dispatch(editTask({ id, changes: { title: 'Edited backlog ' + id } }))}
      ...
      <Dialog open={pickOpen} onClose={() => setPickOpen(false)}>
        ...
        <DialogActions>
          ...
          <Button onClick={handleAssignToDate} variant="contained">Assign</Button>
        </DialogActions>
      </Dialog>
    </Box>
  )
